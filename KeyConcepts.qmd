...
The system will ignore the spatial index if the spatial conditions used specify it must be ignored, for example with the text matching conditions. It can also be turned off (deleted) if needed by using the *Analyses-\>Index* menu. From version 1.0, users can also set the index to be ignored on a per-condition basis. This means it can be used for the first condition, but ignored for the second, or vice versa. This is controlled through the options at the right of the conditions (near the big green tick icon).
...
*Need to explain the specification of extensions and how to load them.*
...
BaseData and Matrix objects can import from text files such as CSV or TXT. They currently guess the field separator by default, but this can be specified as an argument if guesswork is unsuccessful.

Trees can be imported from Nexus files. NeXML support is planned.

Matrices do not care if the matrix is upper right, lower left, both, or any combination of the above. They will store the last defined value for an element pair (reading left to right, top to bottom).
